...
Клиент подключается к серверу. При подключении первого игрока он ждёт (`waiting`). Второй игрок подключается с первым. Сервер пересылает ходы, сигналы о сдаче и предложения ничьи. Логика игры полностью на клиенте — сервер не хранит позицию.
